...
This Emacs configuration is written in a [[https://en.wikipedia.org/wiki/Literate_programming][literate programming]] style,
...
;; Install elpaca-use-package
(elpaca `(,@elpaca-order))

;; to enable the `:elpaca` keyword in use-package
...
To speed up TRAMP access, let’s disable lock files (#files)
#+BEGIN_SRC emacs-lisp
...
Some global default configurations
#+BEGIN_SRC emacs-lisp
...
;; Ignore Heavy Build and Environment Directories (.env, /target ...)
(with-eval-after-load 'projectile
  (dolist (dir '(".venv" "venv" "target" ".build" "node_modules" ".git"))
...
#+BEGIN_SRC emacs-lisp
  (use-package casual :elpaca t)
  ;; casual ibuffer configuration
  (with-eval-after-load 'ibuffer
  (keymap-set ibuffer-mode-map "C-o" #'casual-ibuffer-tmenu)
  (keymap-set ibuffer-mode-map "F" #'casual-ibuffer-filter-tmenu)
  (keymap-set ibuffer-mode-map "s" #'casual-ibuffer-sortby-tmenu))

  ;;casual dired configuration
...
#+BEGIN_SRC emacs-lisp
  (use-package doom-themes
    :elpaca t
    :demand t
    :config (load-theme 'doom-oceanic-next))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package eldoc-mouse :elpaca t
...
Toml/Ansible/Docker/Yaml/Kdl modes
#+BEGIN_SRC emacs-lisp
  ...
  (use-package kdl-mode
    :elpaca (kdl-mode :host github :repo "taquangtrung/emacs-kdl-mode"))
...
Flycheck
#+BEGIN_SRC emacs-lisp
  (use-package flycheck :elpaca t)
#+END_SRC

Devdocs
Documentation viewer for Emacs similar to the built-in Info browser
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package embark-consult
    :elpaca t ; only need to install it, embark loads it after consult if found
    :hook
...
#+BEGIN_SRC emacs-lisp
  (use-package rustic
  :elpaca (rustic :host github :repo "emacs-rustic/rustic")
  ...
  (with-eval-after-load 'org
    ...
    (defalias 'org-babel-prep-session:rust #'org-babel-prep-session:rustic))
  :config
...
;; Install the Org-Babel backend for d2 via Elpaca
(use-package ob-d2
  :elpaca t)

(defun tkj/org-babel-refresh-images ()
  (clear-image-cache)
  (org-redisplay-inline-images))
(add-hook 'org-babel-after-execute-hook #'tkj/org-babel-refresh-images)
  #+END_SRC
...
* Org Mode
#+BEGIN_SRC emacs-lisp
  (global-set-key (kbd "M-p") 'org-edit-src-code) ;; edit source code

  (setq org-return-follows-link t) ;; When at a URL, pressing the <Enter> opens the link in a browser.
  ...
  ;; hide the emphasis marker characters. like * (bold) and / (italic)
  (setq org-hide-emphasis-markers t)

  ;; Prevent Org from altering code indentation when saving or exiting code b
  (setq org-src-preserve-indentation t)

  ;; Make invisible parts of Org elements appear visible at point.
...
#+BEGIN_SRC emacs-lisp
  (use-package org-modern :elpaca t)
...
#+BEGIN_SRC emacs-lisp
(with-eval-after-load 'org
  ;; Load default built-in Babel languages
  ...
  ;; Map json src blocks to json-ts-mode (Emacs 29+)
  (add-to-list 'org-src-lang-modes '("json" . json-ts-mode)))

;; Safely register d2 as soon as ob-d2 is ready on disk
(with-eval-after-load 'ob-d2
  (org-babel-do-load-languages
   'org-babel-load-languages
   (append org-babel-load-languages '((d2 . t)))))

;; Use bash instead of sh for shell blocks
(setq org-babel-sh-command "bash")
#+END_SRC

Org Roam
#+BEGIN_SRC emacs-lisp
  (add-to-list 'load-path "~/utilitaries/org-roam/")
  (add-to-list 'load-path "~/utilitaries/org-roam/extensions/")
  ;; Automatically update #+updated: on save
  (defun my/org-update-updated-timestamp ()
    "Update the #+updated: timestamp in the current Org buffer."
      ...
      (save-excursion
        ...
        (let ((timestamp (format-time-string "[%Y-%m-%d %a %H:%M]")))
          (if (re-search-forward "^#\\+updated:.*$" nil t)
            ...
            ;; Insert field after title if missing
            (when (re-search-forward "^#\\+title:.*$" nil t)
              (end-of-line)
              (insert "\n#+updated: " timestamp)))))))

  (add-hook 'before-save-hook #'my/org-update-updated-timestamp)
